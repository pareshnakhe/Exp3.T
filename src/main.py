...
def module(option, action_set):
    for k in action_set:
        ...
        plt.plot(regret_swucb, 'm', label='SW-UCB')
        plt.plot(regret_exp3r, 'c', label='Exp3.R')
        plt.plot(regret_exp3base, 'b', linewidth=1.0, label='Base Exp3')
        plt.plot(regret_exp3s, 'r', label='Exp3.S')
        plt.plot(regret_exp3d, 'g', linewidth=1.0, label='Exp3D')
        plt.legend(loc='upper left')

        plt.savefig('TEST: HOption={} : Fig-{}-actions.png'.format(option, k))
        plt.clf()
        threadLock.release()
